...
class VideoDescriptionRealTime(object):
    ...
    def main(self, frames_num=80):
        # 1. Initialize reading video object and vgg model
        ...
        while cap.isOpened():
            ...
            if ret:
                ...
                if len(frames) == frames_num:
                    ...
                    fc_feats = model.predict(inputs, batch_size=128)
                    fc_feats = np.array(fc_feats).reshape(1, 80, 4096)
                    
                    # generate inference test outputs
                    if self.search_type == 'greedy':
                        sentence_predicted = self.greedy_search(fc_feats)
                    else:
                        sentence_predicted = ''
                        decoded_sentence = self.decode_sequence2bs(fc_feats)
                    ...
                    caption = '[' + ' '.join(sentence_predicted.split()[1:]) + ']'
